...
class GradCache:
    ...
    def __init__(
        ...
        :param do_cache, bool: use grad_cache
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.do_cache:
            ...
            return self.no_cache_step(*args, **kwargs)
    ...
    def model_call(self, model: nn.Module, model_input):
        ...
        with autocast() if self.fp16 else nullcontext():
            if isinstance(model_input, Tensor):
                model_input = model_input.to(model)
                return model(model_input)
            elif isinstance(model_input, list):
                model_input = list(map(lambda x: x.to(model), model_input))
                return model(*model_input)
            elif isinstance(model_input, (dict, UserDict)):
                return model(**model_input)
            elif isinstance(model_input, tuple) and list(map(type, model_input)) == [list, dict]:
    ...
    def no_cache_step(self,
        ...
        if hasattr(self,'debug'):
            ...
            self.models[0].q_reps = all_reps[0]
