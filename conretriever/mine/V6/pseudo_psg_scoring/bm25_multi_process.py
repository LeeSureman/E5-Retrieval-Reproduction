...
class BM25MultiProcess():
    def __init__(self, dataset_name: str, corpus_path: str, top_k: int=100, use_prebuilt_index: bool=False) -> None:
        """Init class.
        Args
        ----
        dataset_name: str
            Specific name for a task.
        corpus_path: str
            Path to corpus.
        top_k: int=100
            Returns the top k number of passages based on similarity.
        use_prebuilt_index: bool=False
            Whether to use prebuilt index.
        """
        self.top_k = top_k
    ...
    def _multi_process_worker(model, input_queue, results_queue) -> None:
        ...
        """
        if model.use_prebuilt_index:
            logger.info("Use prebuilt index.")
            searcher = LuceneSearcher.from_prebuilt_index(model.corpus_path)
        else:
            logger.info("Use self-built index.")
    ...
    def retrieve(self, queries: List[str], searcher: LuceneSearcher) -> List[List]:
        ...
        end_time = time.time()
        logger.warning(f"One batch took {end_time - start_time} seconds.")
        return docs_list
    ...
    def stop_multi_process_pool(pool: Dict):
        ...
        logger.info("Close all processes!")
