...
def vllm_inference_func(model_name_or_path, prompts, temperature, top_p, max_new_tokens, max_model_len):
    ...
    llm = LLM(model="/remote-home/share/models/mistral_7b_instruct", tensor_parallel_size=torch.cuda.device_count(),
              max_model_len=1024)
    ...
    for output in outputs_texts:
        outputs_vllm.append(output.outputs[0].text)
