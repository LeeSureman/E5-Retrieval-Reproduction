...
def tokenize_into_ids_with_special_token_for_sequence_embedding(input_str, tokenizer, representation_token,
    ...
    input_str_encoded = tokenizer(input_str,
                                  max_length=max_length - representation_special_tokens_num - 2,
...
class RetrievalDataset(Dataset):
    def __init__(self, args_dict, data_root_dir, n_hard_negative, tokenizer: transformers.PreTrainedTokenizer, representation_id,
        ...
        for k in self.raw_data_task_to_dict:
            rank0_print('instruction of task {}: {}'.format(k,
                self.instruction_template.format(self.task_to_query_type[k], self.task_to_passage_type[k])))

        print('dataset[0]:\n{}'.format(self[0]))

    def update_one_epoch_data(self):
        ...
        random.shuffle(this_epoch_data)
        self.raw_data = this_epoch_data

    def __len__(self):
        return len(self.raw_data)

    def wrap_query(self, query, task=None):
...
def train():
    ...
    retrieval_train_dataset = RetrievalDataset(args_dict, data_args.data_root_dir, training_args.n_hard_negative, tokenizer,
