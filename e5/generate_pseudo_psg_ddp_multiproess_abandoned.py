...
def generate_pseudo_psg_process(rank, args, num_process):
    import os
    ...
    for task in args.tasks:
        ...
        output_fp = '{}/{}/shard_{}.jsonl'.format(args.output_dir, task, rank)
        ...
        qid_and_query_s_this_shard = list(filter(lambda x:x[0] % num_process == 0, qid_and_query_s))
        ...
        outputs_vllm = llm.generate(prompts, sampling_params, use_tqdm=(rank == 0))
...
if __name__ == '__main__':
    ...
    mp.spawn(generate_pseudo_psg_process,args=(args,total_gpu_num),nprocs=total_gpu_num,join=True)
    # for i in range(torch.cuda.device_count()):
    #     results.append(pool.apply_async(generate_pseudo_psg_process, (args, i, total_gpu_num)))
    #
    # pool.close()
    # pool.join()
    # for result in results:
    #     print(result.get())
    # generate_pseudo_psg_process(args,0,total_gpu_num)
