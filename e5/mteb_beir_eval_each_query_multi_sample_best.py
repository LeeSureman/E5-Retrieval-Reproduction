...
from V2.replace_mistral_causal_lm_forward import replace_mistral_causal_lm_forward_func
...
parser.add_argument('--need_scores_each_q', type=int, default=1)
# parser.add_argument('--')
...
from V3.my_utils import get_task_instruction_by_task_name
...
class BGERetrievalModel(DRESModel):
    def __init__(self, args):
        ...
        assert 'bge' in args.model_name_or_path, args.model_name_or_path
        # self.bge_model = SentenceTransformer(args.model_name_or_path)
        self.query_instruction = 'Represent this sentence for searching relevant passages: '
        self.bge_model = FlagModel(args.model_name_or_path,
                                   query_instruction_for_retrieval=self.query_instruction, use_fp16=True)
        # self.

        self.gpu_count = torch.cuda.device_count()
        # if self.gpu_count > 1:
        #     pool = self.bge_model.start_multi_process_pool()
        #     self.pool = pool
        # else:
        #     self.pool = None
        # self.encoder = torch.nn.DataParallel(self.encoder)
    ...
    def encode_queries(self, queries, **kwargs):
        original_queries = queries
        input_texts = list(map(lambda x: self.wrap_query(x), queries))
        q_embeddings = self.bge_model.encode_queries(input_texts,
                                                     batch_size=self.args.batch_per_gpu * self.gpu_count)
        q_embeddings = q_embeddings.astype(np.float32)
    ...
    def encode_corpus(self, corpus, **kwargs):
        corpus = ['{} {}'.format(doc.get('title', ''), doc['text']).strip() for doc in corpus]
        input_texts = list(map(lambda x: self.wrap_passage(x), corpus))
        p_embeddings = self.bge_model.encode(input_texts, batch_size=self.args.batch_per_gpu * self.gpu_count)

        return p_embeddings.astype(np.float32)

    def load_pseudo_passage(self, pseudo_psg_data, index=-100):
        query_to_pseudo_psg_dict = {}
        for tmp_js in pseudo_psg_data:
            if index < 0:
                query_to_pseudo_psg_dict[tmp_js['query']] = tmp_js['pseudo_psg']
            else:
                assert type(tmp_js['pseudo_psg']) == type([]), tmp_js['pseudo_psg']
                query_to_pseudo_psg_dict[tmp_js['query']] = tmp_js['pseudo_psg'][index]
...
def main():
    ...
    if args.force_retrieval_model != 'default':
        ...
        print('retrieval model choose RetrievalModel')
        model = RetrievalModel()
    elif 'bge' in args.model_name_or_path:
    ...
    for task in task_names:
        ...
        scores_each_q_list = []
        pseudo_psg_fp = '{}/{}.jsonl'.format(args.pseudo_passage_fp, task)
        pseudo_psg_data = list(jsonlines.open(pseudo_psg_fp))
        assert type(pseudo_psg_data[0]['pseudo_psg']) == type([])
        pseudo_psg_sample_time = len(pseudo_psg_data[0]['pseudo_psg'])

        doc_embedding_cache_dir_this_task = '{}/{}'.format(args.doc_embedding_cache_dir, task)

        doc_embedding_cache_file_name_s = os.listdir(doc_embedding_cache_dir_this_task)
        doc_embedding_indexs = []
        doc_embedding_cache = []
        for fn in doc_embedding_cache_file_name_s:
            e_i = int(fn.split('.')[0].split('_')[1])
            doc_embedding_indexs.append(e_i)

        doc_embedding_cache_num = max(doc_embedding_indexs) + 1
        for e_i in range(doc_embedding_cache_num):
            sub_embedding = torch.load(
                open('{}/embeddings_{}.pkl'.format(doc_embedding_cache_dir_this_task, e_i), 'rb'))
            doc_embedding_cache.append(sub_embedding)

        for pseudo_psg_index in ([-1] + list(range(pseudo_psg_sample_time))):

            if pseudo_psg_index == -1:
                model.query_to_pseudo_psg_dict = None
                logger.info('pseudo_psg_index=-1, so do not use sampled pseudo passage')
            elif args.pseudo_passage_fp != '0':
                model.load_pseudo_passage(pseudo_psg_data, pseudo_psg_index)
            else:
            ...
            evaluation.run(model, eval_splits=["test" if task not in ['MSMARCO'] else 'dev'],
                           output_folder='{}/sample_{}'.format(args.output_dir,pseudo_psg_index), doc_embedding_cache=doc_embedding_cache,
                           need_scores_each_q=args.need_scores_each_q)
            if args.need_scores_each_q:
                scores_each_q_list.append(evaluation.scores_each_q)

        json.dump(scores_each_q_list, open('tmp_scores_each_q_list/{}.json'.format(task), 'w'))
    # if 'bge' in args.model_name_or_path:
    #     if model.pool != None:
    #         model.bge_model.stop_multi_process_pool(model.pool)
